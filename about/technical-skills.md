...
趣味ベースでコンソールアプリケーションや簡単なWebアプリケーション、[DNSベースのアドブロッカー](https://github.com/jyuch/advoid)などを実装するのに使用しています。
...
現在はUnity Catalogを通じたカタログやコンピュートのガバナンスの設計と構築を行っています。

また、基幹システムからデータを連携するためのパイプラインの自動生成などを行っています。

ほかには、PoCのための環境構築やAI/BI Genieのプロンプトエンジニアリングを通じたチューニングなどを行っています。
